...
class Store {
  ...
  /** Removes one tag from many tracks in a single file write — counterpart
   * to addCustomTagToTracks, for removing a tag at album/artist scope. */
  removeCustomTagFromTracks(trackIds, tag) {
    const tags = this.getCustomTags();
    for (const trackId of trackIds) {
      if (tags[trackId]) {
        tags[trackId] = tags[trackId].filter((t) => t !== tag);
