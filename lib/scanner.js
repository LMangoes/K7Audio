...
function buildGenreIndex(tracks) {
  ...
  for (const t of tracks) {
    ...
    for (const custom of t.customTags || []) tags.add(custom);
    if (tags.size === 0) tags.add('UNTAGGED');
    for (const tag of tags) {
      if (!byGenre.has(tag)) byGenre.set(tag, []);
      byGenre.get(tag).push(t);
    }
  }
...
function sortAllSongs(tracks, mode = 'title') {
  ...
  if (mode === 'genre') {
    const effectiveGenre = (t) => t.genre || (t.customTags && t.customTags[0]) || '';
    return copy.sort((a, b) => {
      const ag = effectiveGenre(a);
      const bg = effectiveGenre(b);
      if (ag === '' && bg !== '') return 1;
      if (bg === '' && ag !== '') return -1;
      return (
        ag.localeCompare(bg) ||
